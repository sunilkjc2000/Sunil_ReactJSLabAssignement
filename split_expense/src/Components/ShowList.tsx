...
function ShowData() {
  ...
  const [sunilspent, setSunilspent] = useState<number>(0);
  const [showform, setShowForm] = useState<boolean>(false);

  var rahulspent1: number = 0;
  var rameshspent1: number = 0;
  var sunilspent1: number = 0;
  useEffect(() => {
  ...
  const Shares = (data: IDataList[]) => {
    data.map((sams) =>
      ...
      else if (sams.payeeName === "Sunil")
      {
        sunilspent1 = sunilspent1 + sams.price;
      }
      else
      {
  ...
  return (
    <>
      ...
      <div className="use-inline ">Sunil paid: </div>
      <span className="use-inline total Ramesh">{sunilspent}</span> <br />
